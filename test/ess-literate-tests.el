...
(defun elt--process-case ()
  (let* ((case-beg (point))
    ...
    (condition-case cnd
        (let* ((starting-chunk (elt--get-starting-chunk case-end))
               (test-case-state starting-chunk))
          (while (looking-at (elt--code-chunk-re))
            (setq test-case-state (elt--process-next-chunk case-end test-case-state starting-chunk)))
          (insert "\n")
          (elt--print-section-title)
...
(defun elt--get-starting-chunk (case-end)
...
(defun elt--process-next-chunk (case-end test-case-state test-case)
  (let* ((continuation (looking-at elt-code-chunk-next-re))
         (test-code (elt--process-code case-end))
...
(defun elt--process-code (case-end)
